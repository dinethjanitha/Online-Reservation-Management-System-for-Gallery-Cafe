...
if (isset($_POST['searchsubmit'])) {
    ...
    $query = "SELECT * FROM food_origin WHERE origin_id LIKE '%{$searchval}%' OR origin_description LIKE '%{$searchval}%'
    ...
    if (mysqli_num_rows($result) >= 1) {
        while ($row = mysqli_fetch_assoc($result)) {
            $table .= "<tr>";
            $table .= "<td>{$row['origin_id']}</td>";
            $table .= "<td>{$row['origin_name']}</td>";
            $table .= "<td>{$row['origin_description']}</td>";
            $table .= "<td><a href=\"updatefoodorigin.php?origin_id={$row['origin_id']}\">" .  "<span class=\"badge bg-warning\">Update</span>" . "</a></td>";
            $table .= "<td><a onclick=\"confirm('are you sure?')\" href=\"deletefoodorigins.php?origin_id={$row['origin_id']}\">" .  "<span class=\"badge bg-danger\">Delete</span>" . "</a></td>";
            $table .= "</td>";
        }
    } else {
    ...
    $query = "SELECT * FROM food_origin WHERE is_deleted=0";
