...
if (isset($_POST['searchsubmit'])) {
    ...
    $query = "SELECT * FROM table WHERE table_id LIKE '%{$searchval}%' OR table_description LIKE '%{$searchval}%'
    OR table_name LIKE '%{$searchval}%'";
    ...
    if (mysqli_num_rows($result) >= 1) {
        while ($row = mysqli_fetch_assoc($result)) {
            $table .= "<tr>";
            $table .= "<td>{$row['table_id']}</td>";
            $table .= "<td>{$row['table_name']}</td>";
            $table .= "<td>{$row['table_description']}</td>";
            $table .= "<td>{$row['capacity']}</td>";
            $table .= "<td>{$row['availability']}</td>";
            $table .= "<td><a href=\"updatetable.php?table_id={$row['table_id']}\">" .  "<span class=\"badge bg-warning\">Update</span>" . "</a></td>";
            $table .= "<td><a onclick=\"confirm('are you sure?')\"  href=\"deletetable.php?table_id={$row['table_id']}\">" .  "<span class=\"badge bg-danger\">Delete</span>" . "</a></td>";
            $table .= "</td>";
        }
    } else {
        header("Location: managetable.php?data=notfound");
    }
} else {
    $query = "SELECT * FROM tables WHERE is_deleted=0";
...
<body>
    <div class="container">
        <form action="managefoodcatagories.php" method="post">
            ...
            <div class="row">
                <div class="col-5">
                    <h2>Manage Tables<span><a onclick="confirm('You sure?')" href="./addtables.php">+Add table</a></span> </h2>
                </div>
            </div>
